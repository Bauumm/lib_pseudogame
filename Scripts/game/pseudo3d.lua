...
function PseudoGame.game.Pseudo3D:_update_gradient()
	...
	table.sort(self._edges, function(edge0, edge1)
		...
		if isect_x_range_start >= isect_x_range_end then
			...
			return (get_y_from_x(isect_x_range_start, edge0) - get_y_from_x(isect_x_range_start, edge1) + get_y_from_x(isect_x_range_end, edge0) - get_y_from_x(isect_x_range_end, edge1)) < 0
		end
	end)
	for i=1,edge_index do
		edge = self._edges[i]
