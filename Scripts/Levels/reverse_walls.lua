...
game = PseudoGame.game.Game:new({
	walls = {
		-- make walls go the other way
		reverse_direction = true
	},
	player = {
		-- make the player be on the edge of the screen
		reverse = true,

		-- overwrite the radius the player moves on (negative for inward facing player)
		radius = -400,

		-- reset the collision handler since we overwrote default settings
...
-- or restarted.
function onLoad()
    e_messageAdd("welcome to the eight PseudoGame example level", 150)
    e_messageAdd("Here the wall direction and the player are reversed.", 200)
end
